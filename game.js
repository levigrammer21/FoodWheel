...
const CFG={
  ...
  CHEST_PRICE:500, COMBAT_VICTORY_CLOSE_MS:3000,
};
...
function propertyPrice(id){
  ...
  const owned=countOwned(id);
  return Math.floor(prop.price*Math.pow(1+CFG.PROPERTY_STACK_FEE,owned));
}
function getRentalIncome(){
  ...
  owned.forEach(op=>{
    ...
    const daysElapsed=(now-lastClaim)/(1000*60*60*24);
    total+=Math.floor(prop.price*prop.rentalRate*daysElapsed);
  });
  return total;
}
function claimRent(){
...
function avatarGfx(size=32){
  ...
  if(av&&av.image)return`<img src="${av.image}" alt="" style="width:${size}px;height:${size}px;object-fit:contain;border-radius:4px">`;
  if(av)return av.emoji;
  if(PLAYER_AVATAR.image)return`<img src="${PLAYER_AVATAR.image}" alt="" style="width:${size}px;height:${size}px;object-fit:contain">`;
  return PLAYER_AVATAR.emoji;
}
function avatarGfxFor(plyr,size=24){
  const id=plyr.activeAvatar,av=id?AVATARS.find(a=>a.id===id):null;
  if(av&&av.image)return`<img src="${av.image}" alt="" style="width:${size}px;height:${size}px;object-fit:contain;border-radius:3px">`;
...
function tryAvatarDrop(){
  ...
  if(collected.includes(av.id)){
    ...
    toast(`✨ Duplicate avatar → 🪙${bonus} gold!`);saveP();return;
  }
...
function openAvatarDropModal(av){
  ...
  showModal(`<div style="text-align:center">
    <div style="font-size:0.72rem;color:var(--gold3);font-family:'Cinzel',serif;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.5rem">✨ Avatar Unlocked!</div>
    <div style="width:80px;height:80px;margin:0 auto 0.5rem;display:flex;align-items:center;justify-content:center">${imgHtml}</div>
    <div style="font-family:'Cinzel',serif;font-size:1.05rem;color:${color};margin-bottom:0.2rem;font-weight:700">${av.name}</div>
    <div style="font-size:0.7rem;color:${color};text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.5rem;font-weight:700">${av.rarity}</div>
...
function equipAvatar(id){P.activeAvatar=id;saveP();updateHdr();closeModal();toast("✨ Avatar equipped!");if(TAB==="you")renderYou();}
...
function newPlayer(username){
  return{username,level:1,exp:0,gold:200,bank:0,diamonds:10,
    ...
    avatars:[],activeAvatar:null,createdAt:Date.now()};
}
...
function showTab(tab){
  ...
  if(tab==="walk"){
    ...
    updateWalkUI();startWalkRegenTimer();
    renderWalkFeed();return;
  }
  walkEl.classList.remove("active");
  if(walkRegenInterval){clearInterval(walkRegenInterval);walkRegenInterval=null;}
  ["home","gear","market","social"].forEach(t=>{const b=document.getElementById("nav-"+t);if(b)b.classList.remove("active");});
...
function updateWalkUI(){
  ...
  if(areaDisplay){
    if(CURRENT_AREA){areaDisplay.textContent=`${CURRENT_AREA.emoji} ${CURRENT_AREA.name}`;areaDisplay.classList.add("selected");}
...
function selectArea(id){CURRENT_AREA=WALK_AREAS.find(a=>a.id===id)||null;closeModal();updateWalkUI();}
  ...
  document.getElementById("content").innerHTML=`
    <div class="player-banner">
      ...
      <div class="p-info">
        ...
        <div class="p-class">Level ${P.level} · <span style="color:${tier.color};font-weight:700">${tier.name} Arena</span></div>
        <div class="bar-wrap">
    ...
    <div style="background:#fffbeb;border:1.5px solid #fcd34d;border-radius:12px;padding:0.85rem;margin-bottom:0.7rem;display:flex;align-items:center;gap:0.75rem">
      ...
      <div style="flex:1">
        <div style="font-family:'Cinzel',serif;font-size:0.82rem;color:var(--gold3);font-weight:700">${P.statPoints} Stat Point${P.statPoints>1?"s":""} Available!</div>
        <div style="font-size:0.75rem;color:var(--text3)">Spend them to grow stronger</div>
      </div>
    ...
    <div style="background:#f0fdf4;border:1.5px solid #86efac;border-radius:12px;padding:0.85rem;margin-bottom:0.7rem;display:flex;align-items:center;gap:0.75rem">
      ...
      <div style="flex:1">
        <div style="font-family:'Cinzel',serif;font-size:0.82rem;color:var(--green);font-weight:700">Rental Income Ready!</div>
    ...
    <div class="two-col" style="margin-bottom:0.5rem">
      ...
      <button class="btn btn-purple btn-sm" style="width:100%;padding:0.7rem" onclick="G.showTab('arena')">⚔️ Arena</button>
    </div>
    <div class="two-col">
...
function renderProperties(){
  ...
  document.getElementById("content").innerHTML=`
    ${rentalPending>0?`<div style="background:#f0fdf4;border:1.5px solid #86efac;border-radius:12px;padding:0.85rem;margin-bottom:0.7rem;display:flex;align-items:center;gap:0.75rem">
      <div style="font-size:1.5rem">🏠</div>
      <div style="flex:1"><div style="font-family:'Cinzel',serif;font-size:0.82rem;color:var(--green);font-weight:700">🪙${fmt(rentalPending)} in Rental Income!</div>
        <div style="font-size:0.75rem;color:var(--text3)">${owned.filter(o=>o.instanceId!==P.homePropertyInstanceId).length} rented</div>
      </div>
      <button class="btn btn-green btn-sm" onclick="G.claimRent()">Collect</button>
    </div>`:""}
...
function buyProperty(id){
  ...
  saveP();toast(`🏠 Purchased ${prop.name}!`);renderProperties();
}
function setHome(instanceId,propId){
  P.homePropertyInstanceId=instanceId;P.homePropertyId=propId;
  const maxE=calcMaxEnergy();P.energy=Math.min(P.energy,maxE);
...
function sellProperty(instanceId,propId){
  ...
  showModal(`<div class="modal-title">Sell Property?</div>
    ...
    <div style="text-align:center;color:var(--text3);font-size:0.88rem;margin-bottom:1rem">
      Sell <strong>${prop.name}</strong> for <strong style="color:var(--gold3)">🪙${fmt(sellPrice)}</strong>?
    </div>
    <div class="modal-actions">
...
function confirmSellProperty(instanceId,propId){
  ...
  saveP();closeModal();toast(`🪙 Sold!`);renderProperties();
}
...
function claimQuest(id){
  ...
  checkLevelUp();saveP();toast(`🎁 Claimed! +${q.reward.exp} EXP · +${q.reward.gold}🪙`);renderQuests();
}

// ── ARENA ─────────────────────────────────────────────────────
function renderArena(){
  const tierIdx=arenaT(P.arenaWins||0),tier=ARENA_TIERS[tierIdx],nextTier=ARENA_TIERS[tierIdx+1];
  ...
  document.getElementById("content").innerHTML=`
    <div class="card">
      <div class="arena-tier">
        ...
        ${nextTier?`<div class="bar bar-arena" style="margin:0.5rem auto;max-width:200px"><div class="bar-fill" style="width:${pct}%"></div></div>
          <div style="font-size:0.72rem;color:var(--text3)">${wins}/${nextTier.wins} to ${nextTier.name}</div>`
          :`<div style="font-size:0.82rem;color:var(--gold3);font-weight:700">👑 Maximum rank achieved!</div>`}
      </div>
    </div>
    <div class="card">
      <div class="card-title">📊 Tier Bonuses</div>
      <div class="modal-row"><em>EXP Bonus</em><span style="color:var(--green2);font-weight:700">×${tier.expBonus.toFixed(2)}</span></div>
      <div class="modal-row"><em>Gold Bonus</em><span style="color:var(--gold3);font-weight:700">×${tier.goldBonus.toFixed(2)}</span></div>
    </div>
    <div class="card">
      <div class="card-title">⚔️ Fight</div>
      <div style="font-size:0.85rem;color:var(--text3);margin-bottom:0.75rem">Costs ${CFG.ARENA_COST_GOLD}🪙 and ${CFG.ARENA_COST_EP} energy.</div>
      <button class="btn btn-purple" onclick="G.startArenaBattle()">⚔️ Fight (${CFG.ARENA_COST_GOLD}🪙)</button>
    </div>
    <button class="btn btn-ghost" onclick="G.showTab('home')" style="margin-top:0.5rem">← Back</button>`;
}
function startArenaBattle(){
  ...
  const{str:eStr,def:eDef}=equipStats(P.equipped);
  const tStr=(P.baseStr||10)+eStr,tDef=(P.baseDef||5)+eDef;
  const m={name:`${tier.name} Champion`,emoji:["🗡️","⚔️","🏹","🛡️","🔱","💀","👑"][tierIdx],image:"",
    desc:`A ${tier.name} League champion.`,
    str:Math.round(tStr*(0.8+Math.random()*0.6)),def:Math.round(tDef*(0.8+Math.random()*0.6)),
    hp:P.maxHp+Math.round(P.maxHp*tierIdx*0.15),maxHp:P.maxHp+Math.round(P.maxHp*tierIdx*0.15),
    expReward:Math.round(100*tier.expBonus*(1+tierIdx*0.2)),goldReward:Math.round(150*tier.goldBonus*(1+tierIdx*0.2)),
    isArena:true,tierIdx};
  saveP();updateHdr();openCombatModal(m);
}
...
function doDeposit(){
  ...
  if(amt>(P.gold||0)){toast("Not enough gold!");return;}
  P.gold-=amt;P.bank=(P.bank||0)+amt;saveP();toast(`🏦 Deposited ${fmt(amt)} gold`);renderBank();
}
function doWithdraw(){
  const amt=Math.floor(Number(document.getElementById("withdraw-amt").value));
  if(!amt||amt<1){toast("Enter a valid amount");return;}
  if(amt>(P.bank||0)){toast("Not enough in bank!");return;}
  P.bank-=amt;P.gold=(P.gold||0)+amt;saveP();toast(`🪙 Withdrew ${fmt(amt)} gold`);renderBank();
}
...
function sellToNpc(idx){
  ...
  saveP();toast(`🛒 Sold for 🪙${npcVal}`);renderMarketSell();
}

// ── MYSTERY CHEST ─────────────────────────────────────────────
function openMysteryChest(){
  if((P.gold||0)<CFG.CHEST_PRICE){toast(`💰 Need 🪙${CFG.CHEST_PRICE}!`);return;}
  ...
  if(roll<0.60){
    ...
    reward={emoji:pet.emoji,image:pet.image,name:pet.name,
      sub:`+${pet.val} ${pet.stat==="str"?"STR":"DEF"} · ${pet.rarity} Pet`,
      color:RARITY_COLOR[pet.rarity],extra:""};
  }else{
    const av=rollAvatar(),collected=P.avatars||[];
    if(collected.includes(av.id)){
...
async function buyListing(id,price){
  ...
  await removeListing(id);saveP();toast(`✅ Bought ${listing.item.name}!`);
...
function buyShopItem(kind,idx){
  ...
  saveP();updateQuestProgress("items");toast(`🛒 Bought ${item.name}!`);renderMarketShop();
}
function buyConsumable(id){
  const c=SHOP_CONSUMABLES.find(x=>x.id===id);if(!c)return;
  if((P.gold||0)<c.price){toast("💰 Not enough gold!");return;}
  P.gold-=c.price;applyConsumable(c.effect);saveP();toast(`${c.emoji} Used ${c.name}!`);updateHdr();
...
function lbTab(type){
  document.querySelectorAll("#lb-tabs .tab-btn").forEach((b,i)=>{
    const types=["level","kills","arena","steps","gold","items"];b.classList.toggle("active",types[i]===type);
  });
...
function takeStep(){
  ...
  if(combatState&&!combatState.done){toast("⚔️ Finish your current battle first!");return;}
  if(!CURRENT_AREA){toast("Select an area first!");openAreaSelect();return;}
  ...
  if(roll<CFG.MONSTER_CHANCE){
    ...
    addFeed("🪙","",`Found gold in ${area.name}!`,`+${g} 🪙`,"var(--gold3)");
...
function renderWalkFeed(){
  ...
  if(feed.length===0){feedEl.innerHTML=`<div class="feed-empty">Select an area below, then tap ⚔️ to explore!</div>`;return;}
  feedEl.innerHTML=feed.map(f=>`<div class="feed-item">
...
function resolveChoice(evtId,choiceIdx){
  ...
  if(goldDelta!==0){P.gold=Math.max(0,(P.gold||0)+goldDelta);toast(goldDelta>0?`🪙 +${goldDelta} gold!`:`🪙 Lost ${Math.abs(goldDelta)} gold!`);}
  if(hpDelta!==0){P.hp=clamp((P.hp||0)+hpDelta,1,P.maxHp);toast(hpDelta>0?`❤️ +${Math.abs(hpDelta)} HP!`:`💔 -${Math.abs(hpDelta)} HP!`);}
  saveP();updateHdr();addFeed(evt.emoji,"",msg,choice.label,"var(--text2)");
  showModal(`<div style="text-align:center;font-size:3rem;margin-bottom:0.4rem">${evt.emoji}</div>
    <div class="modal-title">${evt.title}</div>
...
function spawnMonster(area){
  const bonusLevel=area?area.monsterLevelBonus:0,effectiveLevel=P.level+bonusLevel;
...
function abandonCombat(){
  ...
  saveP();updateHdr();closeModal();updateWalkUI();
...
function fleeCombat(){
  ...
  renderCombatModal();saveP();updateHdr();updateWalkUI();toast(`🏃 Escaped! Dropped ${goldLost}🪙.`);
}
function healInCombat(){
  ...
  if(!canAffordSmall){toast("💰 Can't afford potions!");return;}
...
function handleVictory(cs){
  const m=cs.monster,isArena=m.isArena||false;
  ...
  P.npcKills=(P.npcKills||0)+1;P.gold=(P.gold||0)+goldGain;P.exp=(P.exp||0)+expGain;P.hp=cs.playerHp;
...
function checkLevelUp(){
  ...
  if(leveled){P.hp=P.maxHp;toast(`🎉 LEVEL UP! Now Level ${P.level}! +${levels} stat point${levels>1?"s":""}!`);updateHdr();}
}
...
function openItemModal(source,idx){
  ...
  showModal(`<div class="modal-icon">${gfx(item.image,item.emoji,72)}</div>
    ...
    <div class="modal-rarity" style="color:${color}">${item.rarity}
      <span style="margin-left:0.5rem;color:${q.color};font-size:0.65rem;font-weight:700">${q.label}</span></div>
...
window.G={
  ...
  showTab,hideWalk,takeStep,closeModal,openAreaSelect,selectArea,
